    ...
    <div class="relative p-4 w-full max-w-2xl max-h-full mx-auto">
        ...
        <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
            ...
            <div class="flex justify-between items-center pb-4 mb-4 rounded-t border-b sm:mb-5 dark:border-gray-600">
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Nilai {{ $nilai->milik->nama }} on mapel {{ $nilai->mapel->nama_mapel }} by {{ $nilai->diawasi->name }}</h3>
            </div>
            <!-- Modal body -->
            <form action="/nilai/{{ $nilai['id'] }}" method="POST">
                ...
                <fieldset>
                    ...
                    <div class="px-6 py-3">
                        ...
                        <input type="number" name="nilai" id="nilai" value="{{ old('nilai') ?? $nilai['nilai'] }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-20 sm:w-24 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Score">
                    </div>
                    <div class="px-6 py-3">
                        <label for="catatan" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Catatan</label>
                        <textarea name="catatan" rows="3" class="w-full min-h-[40px] text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 
                                    focus:ring-primary-500 focus:border-primary-500 
                                    dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white"
                    placeholder="Write note here">{{ old('catatan') ?? $nilai['catatan'] }}</textarea>
                    </div>
                </fieldset>
