            ...
            <form action="/dashboard/{{ $user['slug'] }}" method="POST">
                        ...
                        <div class="grid grid-cols-5 gap-2.5">
                            @foreach (App\Models\Mapel::get() as $mapel)
                                <div class="flex items-center w-full">
                                    <input id="checkbox-{{ $mapel->id }}" type="checkbox" name="mapel[]"
                                        value="{{ $mapel->id }}"
                                        {{ in_array($mapel->id, old('mapel', $user->mengajar->pluck('id')->toArray())) ? 'checked' : '' }}
                                        class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded-sm">
                                    <label for="checkbox-{{ $mapel['id'] }}"
                                        class="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">{{ $mapel['nama_mapel'] }}</label>
                                </div>
                            @endforeach
                        </div>
                        @error('mapel')
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <div class="grid grid-cols-5 gap-2.5">
                            @foreach (App\Models\Kelas::get() as $kelas)
                                <div class="flex items-center w-full">
                                    <input id="checkbox-{{ $kelas->id }}" type="checkbox" name="kelas[]"
                                        value="{{ $kelas->id }}"
                                        {{ in_array($kelas->id, old('kelas', $user->wali->pluck('id')->toArray())) ? 'checked' : '' }}
                                        class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded-sm">
                                    <label for="checkbox-{{ $kelas['id'] }}"
                                        class="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">{{ $kelas['kelas'] }}</label>
                                </div>
                            @endforeach
                        </div>
                        @error('kelas')
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <a href="/dashboard"
                        ...
                        <svg class="mr-1 -ml-1 w-5 h-5" fill="currentColor" viewBox="0 0 32 32" version="1.1"
