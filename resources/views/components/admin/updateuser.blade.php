                    ...
                    <div class="sm:col-span-2">
                        ...
                        <div class="grid grid-cols-5 gap-2.5">
                            @foreach (App\Models\Kelas::get() as $kelas)
                                <div class="flex items-center w-full">
                                    <input id="checkbox-{{ $kelas->id }}" type="checkbox" name="kelas[]" value="{{ $kelas->id }}" {{ in_array($kelas->id, old('kelas', $user->wali->pluck('id')->toArray())) ? 'checked' : '' }} class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded-sm">
