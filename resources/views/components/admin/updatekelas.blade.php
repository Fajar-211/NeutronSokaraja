    ...
    <div class="relative p-4 w-full max-w-2xl max-h-full mx-auto">
        ...
        <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
            ...
            <div class="flex justify-between items-center pb-4 mb-4 rounded-t border-b sm:mb-5 dark:border-gray-600">
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Update class</h3>
            </div>
            <!-- Modal body -->
            <form action="/kelas/{{ $kelas['kelas'] }}" method="POST">
                @csrf
                @method('PATCH')
                <div class="grid gap-4 mb-4 sm:grid-cols-2">
                    <div>
                        <label for="kelas" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Kelas</label>
                        <input type="text" name="kelas" value="{{ old('kelas') ?? $kelas['kelas'] }}" id="kelas" class="@error('kelas') bg-red-50 border border-red-500 text-red-900 placeholder-red-700 @enderror bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="new class">
                        @error('kelas')
                    ...
                    <div class="flex gap-5">
                        @foreach (App\Models\Category::get() as $mapel)
                            <div class="flex items-center mb-4">
                                <input id="country-option-2" type="radio" name="category" value="{{ $mapel['id'] }}" @checked(old('category', $kelas->category_id ?? null) == $mapel->id) class="w-4 h-4 border-gray-300 focus:ring-2 focus:ring-blue-300 dark:focus:ring-blue-600 dark:focus:bg-blue-600 dark:bg-gray-700 dark:border-gray-600">
