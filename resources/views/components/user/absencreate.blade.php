                  ...
                  <h5>
                      ...
                      @php
                          ...
                          foreach($mapels as $mapel):
                                $mape[] = $mapel['id'];
                            endforeach;
                            $hadirIds = DB::table('temporary_nilais')
                            ->where('user_id', '=', Auth::id())->where('absensi', '=', 'hadir')
                            ->pluck('siswa_id')
                            ->toArray();
                      @endphp
        ...
        <form action="/absent" method="POST">
            ...
            <input type="hidden" name="mapel" value="{{ $mape[0] }}">
              <div class="flex flex-col flex-shrink-0 space-y-3 md:flex-row md:items-center lg:justify-end md:space-y-0 md:space-x-3">
                    <button type="submit" name="action" value="save" class="flex items-center justify-center px-4 py-2 text-sm font-medium text-white rounded-lg bg-teal-500 hover:bg-teal-800 focus:ring-4 focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800">
                      Save
                    </button>
                    <button type="submit" name="action" value="create" class="flex items-center justify-center px-4 py-2 text-sm font-medium text-white rounded-lg bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800">
                        ...
                        <input type="hidden" name="all[]" value="{{ $siswa['id'] }}">
                            <td class="w-4 px-4 py-3">
                                <div class="flex items-center">
                                    ...
                                    // jika temporary ada isinya → ambil status dari temporary table
                                    if (!empty($hadirIds)) {
                                        $checked = in_array($siswa->id, $hadirIds);
                                    }

                                    // kalau user submit & gagal validasi → pakai old()
                                    if (old('siswa')) {
                                        $checked = in_array($siswa->id, old('siswa', []));
                                    }
                                @endphp
                                <input
                                    name="siswa[]"
