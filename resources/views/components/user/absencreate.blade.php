<section class="py-3 sm:py-5">
  <div class="px-4 mx-auto max-w-screen-2xl lg:px-6">
      <div class="relative overflow-hidden bg-white shadow-md dark:bg-gray-800 sm:rounded-lg">
        ...
        @if ($kelas == 'SMA BANYUMAS')
        <form action="/absentBMS" method="POST">
            @csrf
            <input type="hidden" name="pengajar" value="{{ Auth::user()->id }}">
            <input type="hidden" name="mapel" value="{{ $mape[0] }}">
              <div class="flex flex-col flex-shrink-0 space-y-3 md:flex-row md:items-center lg:justify-end md:space-y-0 md:space-x-3">
                @if (empty($hadirIds))
                    <button type="submit" class="flex items-center justify-center px-4 py-2 text-sm font-medium text-white rounded-lg bg-teal-500 hover:bg-teal-800 focus:ring-4 focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800">
                      Save
                    </button>
                    @else
